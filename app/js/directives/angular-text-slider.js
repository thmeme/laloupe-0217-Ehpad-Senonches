  ...
  angular.module('textSizeSlider', [])
    ...
    .directive('textSizeSlider', ['$document', function ($document) {
      return {
        ...
        template: '<div class="text-size-slider"><span class="small-letter" ng-style="{ fontSize: min + unit }">A</span> <input type="range" min="{{ min }}" max="{{ max }}" ng-model="textSize" class="slider" /> <span class="big-letter" ng-style="{ fontSize: max + unit }">A</span></div>',
        // Isolated scope
        scope: {
          min: '@',
          max: '@',
